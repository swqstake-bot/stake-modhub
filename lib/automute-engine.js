...
function previewAutomute(message, rules, { username, strikes = {}, site } = {}) {
  ...
  const strikeKey = uname ? `${siteKey}|${rule.id}|${uname}` : '';
...
class AutoMuteEngine {
  ...
  _pushLog(entry) {
    ...
    const site = entry.site === 'eu' ? 'eu' : 'com';
    const dir = typeof this.getDataDir === 'function' ? this.getDataDir(site) : null;
    if (dir) {
