...
class AutoMuteEngine {
  constructor({ getSettings, getDataDir, resolveUserId, muteUser, sendChatAnnounce, getModNames, onAction, getRelay }) {
  ...
  enqueue(job) {
    ...
    const queueKey = `${job.rule.id}|${uname}`;
    if (this.queue.some((q) => `${q.rule.id}|${q.username.toLowerCase()}` === queueKey)) return;

    const msgKey = normalizeForAutomute(job.message).slice(0, 80);
    const actionKey = `${job.rule.id}|${uname}|${msgKey}`;
  ...
  async _execute({ username, message, rule }) {
    ...
    try {
      ...
      if (chatText && this.sendChatAnnounce) {
        try {
          await this.sendChatAnnounce(chatText);
        } catch (e) {
