...
function parseChatMessage(cm, convRates = {}) {
  ...
  const flags = pickUserFlags(cm);
  const roles = pickUserRoles(cm);
  ...
  if (typename.includes('Tip') || cm?.data?.tip) {
    ...
    const tipRoles = (cm?.data?.tip?.sendBy?.roles || cm?.data?.tip?.sender?.roles || cm?.user?.roles || [])
      .map((r) => r?.name)
      .filter(Boolean);
    return {
      username: tipUser,
      ...
      timestamp: ts,
      flags,
      roles: tipRoles.length ? tipRoles : roles
    };
  }
  if (
