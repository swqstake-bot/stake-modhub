...
const BETS_FILE_RE = /Bets_(de|eu)\.csv$/i;

/** File suffix: stake.com / mirrors → `_de` (legacy), stake.eu → `_eu`. */
function siteLogSuffix(site) {
  return site === 'eu' ? 'eu' : 'de';
}

function normalizeLogSite(site) {
...
function chatLogFilename(site) {
  return `${daySuffix()}Chat_${siteLogSuffix(site)}.csv`;
}

function rainLogFilename(site) {
  return `${daySuffix()}Rain_${siteLogSuffix(site)}.csv`;
}

/** Wie StakeModHub.exe: nur Text/Bot/Tip im Chat-Log (Rain separat). Site: com→Chat_de, eu→Chat_eu. */
function appendChatLog(dir, user, message, site) {
  const ts = new Date().toLocaleString('de-DE');
  appendCsvLine(
    dir,
    chatLogFilename(site),
    'Date;User;Message',
    `${ts};${escCsvCell(user)};${escCsvCell(message)}`
  );
}

function appendRainLog(dir, payload, site) {
  const ts = new Date().toLocaleString('de-DE');
...
function parseBetsCsvLines(lines, byKey, site = 'com') {
  ...
  for (const line of lines) {
    ...
    const betId = cols[1] || '';
    if (!betId) continue;
    const key = registryKey(betId, siteKey);
...
function loadBetsLog(dir, limit = 3000, site = 'com') {
  if (!dir) return [];
  ...
  for (const filename of files) {
    ...
    const fileSite = /Bets_eu\.csv$/i.test(filename) ? 'eu' : siteKey;
    parseBetsCsvLines(lines.slice(1), byKey, fileSite);
  }
...
function logLiveMessage(dir, m, site) {
  if (!m || !m.username) return;
  const siteKey = normalizeLogSite(site || m.site || m.chatSource);
  const kind = m.kind || 'text';
  if (kind === 'rain') {
    appendRainLog(dir, m.rain || { username: m.username, message: m.message }, siteKey);
    return;
  }
  let line = m.message || '';
  if (kind === 'tip') line = line.startsWith('[TIP]') ? line : `[TIP] ${line}`;
  if (kind === 'bot') line = `[BOT] ${line}`;
  appendChatLog(dir, m.username, line, siteKey);
}
