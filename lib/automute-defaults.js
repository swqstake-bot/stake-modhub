...
function normalizeRule(rule) {
  const durations = Array.isArray(rule.durationsMinutes)
    ? rule.durationsMinutes.map((n) => Math.max(1, Number(n) || 10))
    : [10, 60, 1440];
  return {
    ...
    durationsMinutes: durations.length ? durations : [10, 60, 1440],
