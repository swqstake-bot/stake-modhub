...
function parseAutomuteTimestamp(ts) {
...
function loadAutomuteLog(dir, limit = 50) {
  const rows = readCsvBody(dir, 'Automute_Log.csv');
  const out = rows
    .map((c) => ({
      at: parseAutomuteTimestamp(c[0]),
      username: c[1] || '',
      ruleLabel: c[2] || '',
      strike: c[3] ? Number(c[3]) : null,
      expire: c[5] || '',
      dryRun: c[6] === 'ja',
      ok: c[7] === 'ok',
      skipped: c[7] !== 'ok' ? c[7] : null,
      muteReason: c[8] || '',
      preview: c[9] || ''
    }))
    .filter((e) => (e.ok || e.dryRun) && e.username);
  out.sort((a, b) => (b.at || 0) - (a.at || 0));
  return out.slice(0, Math.max(1, limit || 50));
}

function automuteLogDedupKey(entry) {
  const atMin = Math.floor((Number(entry?.at) || 0) / 60000);
  const user = String(entry?.username || '')
    .replace(/^@/, '')
    .trim()
    .toLowerCase();
  const preview = String(entry?.preview || '').slice(0, 120);
  const rule = String(entry?.ruleLabel || entry?.ruleId || '');
  const strike = entry?.strike != null ? String(entry.strike) : '';
  return `${atMin}|${user}|${preview}|${rule}|${strike}`;
}

function mergeAutomuteLogEntries(mem = [], disk = [], limit = 50) {
  const seen = new Set();
  const merged = [];
  for (const entry of [...mem, ...disk]) {
    const key = automuteLogDedupKey(entry);
