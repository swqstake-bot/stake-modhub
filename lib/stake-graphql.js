...
const { normalizeStakeMultiplier } = require('./stake-multiplier');
...
class StakeGraphQL {
  ...
  async getPublicChats() {
    ...
    return this.query(query, {}, 'PublicChats');
  }
  ...
  /** Mehrere API-Seiten laden (Stake liefert typisch ~50 pro Request). */
  async getUserChatHistoryAll(name, { pageSize = 50, maxItems = 200 } = {}) {
    if (!name) return null;
    const capped = Math.max(1, Math.min(500, Number(maxItems) || 200));
    ...
    while (chatHistory.length < capped) {
      const limit = Math.min(size, capped - chatHistory.length);
      const data = await this.getUserChatHistory(name, limit, offset);
      if (!data?.user) break;
      user = data.user;
      const batch = data.user.chatHistory || [];
      ...
      for (const item of batch) {
        ...
        added++;
        if (chatHistory.length >= capped) break;
      }
      offset += batch.length;
    ...
    if (!user) return null;
    return { user: { id: user.id, name: user.name, chatHistory } };
  }
  ...
  extractBetDetail(betWrap) {
    ...
    return {
      iid: betWrap.iid,
      game: gameName,
      multiplier: normalizeStakeMultiplier(payoutMultiplier),
      hidden,
