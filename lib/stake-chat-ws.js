...
class StakeChatWebSocket {
  constructor(handlers = {}) {
    ...
    /** Optional Chromium transport (avoids Cloudflare 403 on Node ws). */
    this.chromium = handlers.chromium || null;
    this.ipcMain = handlers.ipcMain || null;
    this.ws = null;
    this.pingTimer = null;
    ...
    this._transport = 'node'; // 'node' | 'chromium'
    this._chromiumFailCount = 0;
  }

  isConnected() {
    if (this._transport === 'chromium') return this.connected;
  ...
  start(config) {
    ...
    // Prefer Chromium when available — Node ws is frequently CF-blocked (403).
    if (this.chromium && this.ipcMain) {
      this._connectChromium();
    } else {
      this._connectNode();
    }
  }
  ...
  async _connectChromium() {
    this._transport = 'chromium';
    this.lastError = '';
    this.lastGraphqlError = '';
    this.connected = false;
    this.subscribed = false;
    this.emitStatus({ phase: 'connecting', transport: 'chromium', wsHost: this.config?.wsHost });
    try {
      await this.chromium.start(this.config, this.ipcMain);
    } catch (e) {
      this.lastError = e?.message || String(e);
      this.emitStatus({ phase: 'error', error: this.lastError, transport: 'chromium' });
      this._chromiumFailCount += 1;
      if (this.shouldRun && this._chromiumFailCount < 2) {
        this._connectNode();
      } else {
  ...
  _onChromiumStatus(st = {}) {
    const phase = st.phase || '';
    if (phase === 'connecting' || phase === 'open') {
      this.connected = phase === 'open';
      this.emitStatus({ ...st, transport: 'chromium' });
      return;
    }
    if (phase === 'subscribed') {
      this.connected = true;
    ...
    if (phase === 'graphql_error') {
      this.lastGraphqlError = st.error || '';
      this.emitStatus({ phase: 'graphql_error', error: this.lastGraphqlError, transport: 'chromium' });
      return;
    }
    if (phase === 'error' || phase === 'closed') {
      this.connected = false;
      this.subscribed = false;
      if (st.error) this.lastError = st.error;
      else if (st.code) this.lastError = `closed ${st.code} ${st.reason || ''}`.trim();
      this.emitStatus({ ...st, transport: 'chromium' });
      this._scheduleReconnect();
    }
  }

  _onChromiumFrame(obj) {
    if (!obj) return;
    try {
      ...
      const batch = parseWsFrame(JSON.stringify(obj.payload || obj), this.convRates);
  ...
  _connect() {
    ...
    // Attach clearance under the configured name; also mirror as cf_clearance when
    // settings still say Non Permanent but CF now requires clearance for WS.
    if (clearance) {
      cookieParts.push(`${cfName}=${clearance}`);
      if (cfName !== 'cf_clearance') cookieParts.push(`cf_clearance=${clearance}`);
    }
    ...
    this.emitStatus({ phase: 'connecting', url, wsHost: host, transport: 'node' });
    ...
    this.ws.on('open', () => {
      this.connected = true;
      this.emitStatus({ phase: 'open', wsHost: host, transport: 'node' });
      const init = {
    ...
    this.ws.on('close', (code, reason) => {
      ...
      this.emitStatus({ phase: 'closed', code, reason: reasonStr, transport: 'node' });
      this._scheduleReconnect();
    });

    this.ws.on('error', (err) => {
      this.lastError = err.message;
      this.emitStatus({ phase: 'error', error: err.message, transport: 'node' });
      if (/403/.test(err.message || '') && this.chromium && this.ipcMain && this.shouldRun) {
        this._teardownSocket();
        this._connectChromium();
      }
    });
  ...
  _scheduleReconnect() {
    ...
    this.emitStatus({ phase: 'reconnecting', lastError: this.lastError, transport: this._transport });
    this.reconnectTimer = setTimeout(() => {
      this.reconnectTimer = null;
      if (!this.shouldRun) return;
      if (this.chromium && this.ipcMain && (this._transport === 'chromium' || /403/.test(this.lastError || ''))) {
        this._connectChromium();
      } else {
        this._connectNode();
      }
