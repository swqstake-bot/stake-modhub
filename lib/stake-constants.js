...
/** Per-site Hub settings (RH / automsg / automute / chatroom) — not shared between .com and .eu */
const HUB_SITE_KEYS = [
  'prefChatroom',
  'rhCrashTimerMinutes',
  'rhCrashTimerSeconds',
  'rhDefaultGame',
  'rhAutopostEnabled',
  'rhAutopostIntervalMinutes',
  'rhAutopostStopOnHit',
  'autoMessages',
  'rulePostEnabled',
  'rulePostIntervalMinutes',
  'rulePostExtra',
  'automuteEnabled',
  'automuteDryRun',
  'autoMuteRules'
...
function cloneHubSite(src) {
  const base = { ...DEFAULT_HUB_SITE, ...(src || {}) };
  base.autoMessages = Array.isArray(base.autoMessages)
    ? JSON.parse(JSON.stringify(base.autoMessages))
    : JSON.parse(JSON.stringify(DEFAULT_HUB_SITE.autoMessages));
  base.autoMuteRules = Array.isArray(base.autoMuteRules)
    ? JSON.parse(JSON.stringify(base.autoMuteRules))
    : [];
  return base;
}

function pickHubKeys(obj) {
  const out = {};
  if (!obj || typeof obj !== 'object') return out;
  for (const k of HUB_SITE_KEYS) {
...
const DEFAULT_SETTINGS = {
  ...
  activeSite: 'com',
  /** RH / automsg / automute / chatroom — independent per site */
  hubBySite: {
    com: cloneHubSite(DEFAULT_HUB_SITE),
    eu: cloneHubSite(DEFAULT_HUB_SITE)
  },
  /** Flat mirrors of active hub (kept for older readers / UI); source of truth is hubBySite. */
  ...cloneHubSite(DEFAULT_HUB_SITE),
